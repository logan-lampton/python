...
def display_celebrity(celebrity, position):
  ...
  if description[0] == "a" or description[0] == "e" or description[0] == "i" or description[0] == "o" or description[0] == "u":
...
def high_low():
  ...
  while playing:
    ...
    if follower_count1 > follower_count2:
      ...
      loser = celebrity1
      winner = celebrity2
    if (selection == 'a' and winner == celebrity1) or (selection == 'b' and winner == celebrity2):
      print(f"Correct! 😁 {winner['name']} has {winner['follower_count']}M followers, which is more than {loser['name']}'s {loser['follower_count']}M followers")
      whether_continue = input("Continue to the next question? Type 'Y' or 'N': ").lower()
      ...
      playing = False
      print(f"Sorry! 😟 {loser['name']} has {loser['follower_count']}M followers, which is less than {winner['name']}'s {winner['follower_count']}M followers. You lose.")
      print(f"Number correct: {number_correct}")
